...
import { getTimeStampFromTimeFrame } from "../../constants";
import { TimeSelected, UseDefaultProps } from "../../interfaces/chart";
...
export const useDefault = ({
  ...
  const tooltip = {
    ...
    formatter(params) {
      ...
      // )}</p></div>`;
      return `
      <div style="color:${"rgba(255, 255, 255, 0.8)"};font-weight:600;display:flex;justify-content:space-between"><span>${dateFormatter.format(
        date
      )}</span><span style="font-weight:400;color:${"rgba(255, 255, 255, 0.4)"}; margin-left: 5px">${timeFormatter.format(
        date
      )}</span></div>
      ${params
        .map(
          (param) =>
            `<div style="font-weight:600;display:flex;align:center;margin-top:7.5px;"><div margin-right:5px; style="border-radius:10px;margin-top:3px;padding:2px;background:${"rgba(255, 255, 255, 0.6)"};height:14px;width:fit-content;margin-right:5px;display:flex;align:center"><span style="display:inline-block; width:10px; height:10px; border-radius:50%; background-color:${
              param.color
            };"></span></div> <span style="color:${"rgba(255, 255, 255, 0.4)"};margin-right:5px;font-weight:400">${
              param.seriesName
            }:</span> ${
  ...
  const yAxis = [
    {
      ...
      axisLabel: {
        ...
        formatter: (value: number) =>
          value.toFixed(spreadResolution).toString(),
        show: !noAxis,
      },
      axisPointer: {
        show: false,
        ...
        : transactions
            ...
            ?.map((transaction) => ({
              ...
              label: {
                ...
                rich: {
                  a: {
                    opacity: 1,
                    backgroundColor:
                      transaction.type === "buy" ? "#16C784" : "#ea3943",
                    borderRadius: 12,
                    symbolSize: 15,
              ...
              tooltip: {
                trigger: "item",
                formatter(params) {
                  const tx: any = params.value[2];
                  return `<span style="margin-right:5px;color:${"rgba(255,255,255,0.4)"};">Date:</span> <span style="color=${"rgba(255,255,255,0.8)"};font-weight:600">${
                    tx.timestamp
                  }</span><br/><span style="margin-right:5px;color:${"rgba(255,255,255,0.4)"};">${getVerb(
                    tx.type
                  )} at:</span> <span style="color=${"rgba(255,255,255,0.8)"};font-weight:600">$${
                    tx.amount_usd / tx.amount
                  }</span><br/><span style="margin-right:5px;color:${"rgba(255,255,255,0.4)"};">Amount:</span> <span style="color=${"rgba(255,255,255,0.8)"};font-weight:600">${`${tx.amount} ${tx.asset?.symbol}`}</span>
        <span style="margin-right:5px;color:${"rgba(255,255,255,0.4)"};">($${
                    tx.amount_usd
                  })</span>`;
                },
              },
            })) || [],
    [transactions, data, timeframe, extraData]
  );
